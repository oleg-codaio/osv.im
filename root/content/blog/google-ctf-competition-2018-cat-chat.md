...
date: 2018-06-25
readTime: 10
image: https://miro.medium.com/max/1400/1*iiIHDrFhlePCSfevZP8v0A.png
...
This challenge wasn't trivial, but in the end I was able to get the flag. Read on to find out how!

## Starting Out

Cat Chat reminded me of IRC hacking challenges popular a decade ago, like the ones on HackThisSite. The premise is that there's a chat app run by someone bent against canines, and the goal is to steal the admin's credentials to get the flag.
...
Moving on, the admin really, really hates dogs. As a test, let's see what happens when we join the same room as another user (i.e., in an incognito window) and start talking about dogs. The admin comes in, bans anyone who utters the word, and promptly disconnects. That's interesting — we can summon the admin to our chatroom at any time. Once we figure out what exactly makes the admin an admin, maybe we can somehow compromise them?

Before we go on, let's also look at one more thing: whether the client sets a Content Security Policy. If it does (as any modern web app should), that will significantly decrease our client-side attack vectors.
...
We now know how the admin actually bans unwelcome 🐕 supporters, and more importantly we know how to become the admin. Running the `/secret` command authenticates the user; we'll later discover that using it sets a cookie named `flag` in the browser 🚩. So, to get the flag we have to steal the admin's secret cookie. Easier said than done!

Let's proceed further and look at the client, `catchat.js`. First thing that we notice is the admin is quite lazy, running a function to look for the word "dog" and ban anyone who said it. We know that we can change our name to anything, so let's keep that in mind — the admin will repeat any name that we give it 🚩.

Proceeding further, we can see how all the different types of messages are handled. The code escapes all untrusted user input with an `esc()` function, so we can't just start writing our own HTML. But there are still a few things of interest here:
...
3. Remember how a banned user becomes red in the chat room? Apparently that's done using some CSS that matches all divs whose `data-name` attribute starts with the banned user's name 🚩. Looks like the developer forgot to quote the name though, and there's no good reason to use `^=` instead of `=` for the predicate.

Other than that, the code looks reasonably robust. The rest of it deals with handling reporting user a reCAPTCHA to prevent spamming the admin and generating a new name upon connection. We've gleaned enough from here, so let's hop over and look at the server's source code.
...
1. If the `data-secret` content starts with A, set the background-image to a logged URL containing "A"
2. If the `data-secret` content starts with B, set the background-image to a logged URL containing "B"
3. If the `data-secret` content starts with C, set the background-image to a logged URL containing "C"
... and so on

Then, once we get the first letter (let's say it's C):

1. If the `data-secret` content starts with CA, set the background-image to a logged URL containing "CA"
2. If the `data-secret` content starts with CB, set the background-image to a logged URL containing "CB"
3. If the `data-secret` content starts with CC, set the background-image to a logged URL containing "CC"
... and so on

### Bypassing the CSP

The next step is to actually log these requests, so that we know when we've guessed a letter correctly. The server's CSP actually prevents embedding any external images. But, recall that we can actually send messages to any chatroom via a simple URL. So we could instead use an image source like:
...
Provided that the character set of the flag is restricted (we'll assume uppercase letters, numbers, and `{` and `}` for now), let's try this out on ourselves first. We can't try too many characters at once because URLs can only be so big. Note that to make the rest of the CSS valid we can simply end with a `/*`.

That worked! We successfully injected some CSS to trick the browser into issuing a request to post a message to the chat.
...
While clearly staged, this was a fun challenge and I enjoyed putting together the fabled character-by-character CSS attack. But there are some lessons to take away here:

1. Set a Content Security Policy on your site to offer a last-resort protection against malicious user-injected content, and be extra careful about allowing access to sources you don't control, especially `unsafe-inline`. For inline scripts and styles, use a nonce.
...
3. Follow the REST spec for defining endpoints — GET requests should be idempotent and never mutate any resources.
4. Similar to avoiding `goto`, be extra careful about complex `switch-case` statements, particularly jumping across cases. Doing otherwise can set you up for failure.
