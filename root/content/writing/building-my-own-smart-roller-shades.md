...
<figure>
  <video controls playsinline preload="metadata" poster="/images/writing/building-my-own-smart-roller-shades/shado_poster.jpg">
...
<figure>
  <video controls playsinline preload="metadata" poster="/images/writing/building-my-own-smart-roller-shades/google_home_flow_poster.jpg">
...
| :----------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------ | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **`action.devices.INTENT.SYNC`**<br/>_(Discovery)_     | User links their Shado account in the Google Home app or says _"OK Google, sync my devices."_                       | The backend queries the user's registered devices and returns device metadata (`BLINDS` type with `OpenClose` trait), adding the shades to the Google Home app.                       |
| **`action.devices.INTENT.EXECUTE`**<br/>_(Control)_    | User gives a voice command (_"OK Google, open the blinds"_, _"Set shades to 75%"_) or taps controls in Google Home. | The backend parses the target `openPercent` and updates the AWS Device Shadow (`desired` state). An MQTT delta message triggers the ESP8266 to spin the motor to the target position. |
| **`action.devices.INTENT.QUERY`**<br/>_(Status Check)_ | User asks a status question (_"OK Google, are the blinds open?"_) or opens the device card in the Google Home app.  | The backend reads `getThingShadow()` from AWS IoT Core and returns the current `reported.openPercent` position and online status to Google Assistant.                                 |
...
- My 3D printer produced plenty of failed prints, and some of the larger parts took hours to reprint.
- During IoT fleet provisioning, serializing a 4 KB certificate payload into a JSON object wreaked havoc on the ESP8266's limited heap and occasionally caused crashes.
- My MQTT client also kept disconnecting — turns out the receive buffer was [too small](https://stackoverflow.com/questions/63448172/esp8266-failing-to-receive-message-over-mqtt-via-aws-iot-core).
- AWS Cognito didn't provide a server-side way to resolve an Identity Pool ID from a User Pool profile. As a result, Google Assistant requests could not look up a user's devices until I cached the Identity ID in a custom user attribute during web login.
...
These days, I’d probably build around Matter, integrate with Home Assistant, and experiment with OpenClaw for natural-language control.
